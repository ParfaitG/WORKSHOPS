...
<div style="text-align:center"><img src="images/Gov_UK_National_Statistics.png" alt="UK Gov Screenshot" width="600px"/></div>
...
output <- by(uk_ghg_df, uk_ghg_df$Gas, function(sub) {
  ...
  barplot(dat, beside = TRUE, 
          main = paste("U.K. ", sub$`Gas`[1], "Emissions"),
...
uk_ghg_df <- subset(uk_ghg_df, `Gas Coverage` == "Including net emissions/removals from LULUCF" & 
                               `Gas` == "Total")
uk_ghg_df
```
...
output <- by(uk_ghg_long_df, uk_ghg_long_df$`Geographic Coverage`, function(sub) {
   ...
   points(sub$Year, sub$Value, pch=16, col=seaborn_palette[as.integer(sub$`Geographic Coverage`)]) 
})
...
```{r}
uk_ghg_df <- subset(uk_ghg_tables$`Table 3`[c(1:122),],
                    is.na(More.Detail))
...
with(subset(uk_ghg_long_df, Sector=="Energy supply"), {
  ...
  plot(Year, Value, type='l', pch=16,
       main="UK Total Greehouse Gas Emissions by Sector",
...
output <- by(uk_ghg_long_df, uk_ghg_long_df$Sector, function(sub) {
   ...
   points(sub$Year, sub$Value, pch=16, col=seaborn_palette[as.integer(sub$Sector)]) 
})
